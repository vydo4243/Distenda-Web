...
import { voucherHistoryController } from "../../../controllers/history.controller";
...
export default function VoucherHistory({ onClose }) {
  ...
  useEffect(() => {
    async function fetchData() {
      // console.log("vao")
      const result = await voucherHistoryController(setLoading);
      // console.log(result)
      if (result) {
  ...
  const filteredData = data?.filter((item) => {
    ...
    return (
      ...
      item.voucherCode?.toLowerCase().includes(keyword) ||
      ...
      {filteredData && filteredData.length > 0 ? (
        <div className="flex flex-col gap-3 justify-start self-start w-full mt-4 max-h-[70vh] overflow-y-auto">
          {filteredData.map((history, index) => {
            ...
            const voucherName = history.voucherCode.toUpperCase();
